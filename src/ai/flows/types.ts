...
export const GenerateProductViewInputSchema = z.object({
  ...
  selectedColor: z.string().optional().describe('The user-entered color, if manually provided. Empty means auto-detect.'),
  detectedColor: z.string().optional().describe('The color detected by the SEO/content flow from product inputs or images.'),
  effectiveColor: z.string().optional().describe('The final product color used for image generation, ZIP, and catalog defaults.'),
  isManualColor: z.boolean().optional().describe('Whether the final color came from a user-entered manual color override.'),
